...
# import cmtGratingsTD.cmt_gratings.utils.si_helper as si

# import cmtGratingsTD.cmt_gratings.utils.cplots as cp
...
from cmt_gratings import OUTPUT_DIR
from cmt_gratings.utils import si_helper as si
from cmt_gratings.utils import cplots as cp
...
def main():
    ...
    ###############
    cm.plots = cp.cplots()
    cm.save_dir = f"{OUTPUT_DIR}/moire"
    cm.plots.clean_dir(cm.save_dir)
    ###############

    cm.c = np.float64(299792458)  # speed of light
    cm.epsilon0 = np.float64(8.85418782e-12)
    cm.mu0 = np.float64(1.25663706e-6)
    ...
    for ii in range(0, cm.xv_nopo):
        if cm.xv[ii] >= cm.glen / 2 + cm.xbuffer:
            cm.condv[ii] = cm.cond * (
                ...
                * np.cos(
                    (np.pi / cm.xdamping)
                    * (cm.xv[ii] - (cm.glen / 2 + cm.xbuffer + cm.xdamping))
                )
            )
    ...
    cm.xv_glen = np.linspace(
        -cm.glen / 2, cm.glen / 2, cm.xv_glen_nopo, dtype=np.float64
    )
    ...
    poynting = cm.c * np.real(
        np.conj(cm.modes[0, :]) * cm.modes[0, :]
        - np.conj(cm.modes[1, :]) * cm.modes[1, :]
    )
    edensity = cm.n0 * np.real(
        np.conj(cm.modes[0, :]) * cm.modes[0, :]
        + np.conj(cm.modes[1, :]) * cm.modes[1, :]
    )
    ...
    cm.plots.line_plot(
        cm.xv_glen, cm.gv_func / cm.vp, filename="gv", save_dir=cm.save_dir
    )
    cm.plots.line_plot(cm.xv_glen, cm.gv_apod, filename="gv_apod", save_dir=cm.save_dir)
    cm.plots.line_plot(
        cm.xv_glen, cm.gv_moire, filename="gv_moire", save_dir=cm.save_dir
    )
    ...
    cm.plots.vline = True
    cm.plots.vline_values = [
        cm.t0 - cm.pulse_tlen / 2,
        cm.t0,
        cm.t0 + cm.pulse_tlen / 2,
    ]
    ...
    img = cm.plots.fig2cv(
        cm.plots.line_hoz_split_plot(
            cm.xv, intensity_field_fwd[0, :], cm.xv, intensity_field_bwd[0, :]
        )
    )

    cm.vid_name = f"{cm.save_dir}/cmt_moire_td_gauss_apod.mp4"
    cm.vid_fourcc = cv.VideoWriter_fourcc(*"mp4v")
    cm.vid_out = cv.VideoWriter(
        cm.vid_name, cm.vid_fourcc, fps, (img.shape[1], img.shape[0]), True
    )
    ...
    for ii in range(0, len(sol.t)):
        ...
        img = cm.plots.fig2cv(
            cm.plots.line_hoz_split_plot(
                cm.xv, intensity_field_fwd[ii, :], cm.xv, intensity_field_bwd[ii, :]
            )
        )
        cm.vid_out.write(img)
    cm.vid_out.release()
...
class cm_class:
    def sellmeier_fused_silica(self, l0):
        ...
        n_sqr = (
            1
            + ((0.6961663 * l0**2) / (l0**2 - 0.0684043**2))
            + ((0.4079426 * l0**2) / (l0**2 - 0.1162414**2))
            + ((0.8974794 * l0**2) / (l0**2 - 9.896161**2))
        )
    ...
    def sellmeier_lithium_niobate(self, l0):
        ...
        n_sqr = (
            1
            + ((2.6734 * l0**2) / (l0**2 - 0.01764))
            + ((1.2290 * l0**2) / (l0**2 - 0.05914))
            + ((12.614 * l0**2) / (l0**2 - 474.60))
        )
        return np.sqrt(n_sqr)
    ...
    def tpulse_func(self, t):
        return (
            self.tamp
            * self.tamp_scale
            * np.exp(-0.5 * (self.c**2) * (self.sigma**2) * (t - self.t0) ** 2)
        )

    def xpulse_func(self, t):
        z = t * self.vp
        return self.xpulse_amp * np.exp(
            -0.5 * (self.sigma**2) * (z - self.x0_input) ** 2
        )
    ...
    def dfdt_func(self, t, modes):
        ...
        self.dfdt[0, :] = self.vp * (
            self.source
            - self.dudz
            - (self.loss + self.condv) * self.u0
            - self.kappa * self.apod * self.v0
        )
        self.dfdt[1, :] = self.vp * (
            self.dvdz
            - (self.loss + self.condv) * self.v0
            + self.kappa * self.apod * self.u0
        )
