...
class cplots:
    ...
    def set_vlines(self):
        if self.vline:
            ...
            for ii in range(0, len(self.vline_values)):
                plt.vlines(
                    x=self.x_scale * self.vline_values[ii],
                    ymin=ylims[0],
                    ymax=ylims[-1],
                    lw=self.vline_lw[ii],
                    color=self.vline_color[ii],
                    linestyle=self.vline_style[ii],
                )
    ...
    def set_hlines(self):
        if self.hline:
            ...
            for ii in range(0, len(self.hline_values)):
                plt.hlines(
                    y=self.y_scale * self.hline_values[ii],
                    xmin=xlims[0],
                    xmax=xlims[-1],
                    lw=self.hline_lw[ii],
                    color=self.hline_color[ii],
                    linestyle=self.hline_style[ii],
                )
    ...
    def fig2cv(self, fig):
        fig.canvas.draw()
        img = cv.cvtColor(
            np.array(fig.canvas.renderer.buffer_rgba()), cv.COLOR_RGBA2BGR
        )
        plt.close()
        return img
    ...
    def line_hoz_split_plot(self, x1, y1, x2, y2, *args, **kwargs):
        ...
        if self.vline:
            for ii in range(0, len(self.vline_values)):
                ax1.vlines(
                    x=self.x_scale * self.vline_values[ii],
                    ymin=self.y_lim_values[0],
                    ymax=self.y_lim_values[1],
                    lw=self.vline_lw[ii],
                    color=self.vline_color[ii],
                    linestyle=self.vline_style[ii],
                )
